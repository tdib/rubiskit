...
export function animateCameraRotation(
  ...
  console.log('here')
  let cameraPosition: Vector3 = new THREE.Vector3(6, 6, 6)
  ...
  const startQuaternion = new THREE.Quaternion().setFromRotationMatrix(new THREE.Matrix4().lookAt(startPosition, pivot, axis));
  const endQuaternion = new THREE.Quaternion().setFromRotationMatrix(new THREE.Matrix4().lookAt(endPosition, pivot, axis));
  
  const startUpQuaternion = new THREE.Quaternion().setFromUnitVectors(new THREE.Vector3(0, 1, 0), upVector);
  const endUpQuaternion = new THREE.Quaternion().setFromUnitVectors(new THREE.Vector3(0, 1, 0), determineUpVector(axis));
  ...
  const animate = () => {
    ...
    if (animationProgress < 1) {
      ...
      const direction = new THREE.Vector3(0, 0, 1).applyQuaternion(currentQuaternion);
      ...
      // cameraPosition = direction.multiplyScalar(startPosition.length());
      
      const currentUpQuaternion = new THREE.Quaternion();
      currentUpQuaternion.slerpQuaternions(startUpQuaternion, endUpQuaternion, animationProgress);
      
      const currentUpVector = new THREE.Vector3(0, 1, 0).applyQuaternion(currentUpQuaternion);
      cameraState.update((state) => {
        return {
          ...state,
          up: currentUpVector
